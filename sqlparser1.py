...
class SqlParser(object):
    ...
    def p_preparable_statement(self, p):
        """
        preparable_statement : preparable_SQL_data_statement
        """
        children = p[1]
        sql = p[1].sql
        p[0] = Node('preparable_statement', children, sql)

    def p_preparable_SQL_data_statement(self, p):
        """
        preparable_SQL_data_statement : dynamic_single_row_select_statement
                                      | insert_statement
        """
        children = p[1]
        sql = p[1].sql
        p[0] = Node('preparable_SQL_data_statement', children, sql)

    def p_dynamic_single_row_select_statement(self, p):
        """
        dynamic_single_row_select_statement : query_specification
        """
        children = p[1]
        sql = p[1].sql
        p[0] = Node('dynamic_single_row_select_statement', children, sql)
    ...
    def p_insert_statement(self, p):
        ...
        p[0] = Node('insert_statement', children, sql)

    def p_insert_columns_and_source(self, p):
        """
        insert_columns_and_source : LPAREN insert_column_list RPAREN query_expression
                                  | query_expression
                                  | DEFAULT VALUES
        """
        if len(p) == 2:
            ...
            children = [p[2], p[4]]
            sql = '\n     ( %s) %s' % (p[2].sql, p[4].sql)
        p[0] = Node('insert_columns_and_source', children, sql)
    
    def p_insert_column_list(self, p):
        """
        insert_column_list : column_name_list
        """
        children = p[1]
        sql = p[1].sql
        p[0] = Node('insert_column_list', children, sql)
    ...
    def p_opt_top(self, p):
        """
        opt_top : TOP LPAREN NUMBER RPAREN
                | TOP NUMBER
                |
        """
        children = None
    ...
    def p_derived_column(self, p):
        ...
        sql = '%s%s' % (value_expression.decode('utf8'), as_clause)
    ...
    def p_string_value_expression(self, p):
        """
        string_value_expression : character_value_expression
        """
        children = p[1]
        sql = p[1].sql
        p[0] = Node('string_value_expression', children, sql)

    def p_character_value_expression(self, p):
        """
        ...
        children = p[1]
        sql = p[1].sql
        p[0] = Node('character_value_expression', children, sql)
    ...
    def p_concatenation(self, p):
        ...
        p[0] = Node('concatenation', children, sql)

    def p_concatenation_operator(self, p):
        """
        concatenation_operator : PLUS
        """
        sql = p[1]
        p[0] = Node('concatenation_operator', None, sql)

    def p_character_factor(self, p):
        """
        ...
        p[0] = Node('character_factor', children, sql)

    def p_character_primary(self, p):
        """
        ...
        children = p[1]
        sql = p[1].sql
        p[0] = Node('character_primary', children, sql)

    def p_string_value_function(self, p):
        """
        string_value_function : character_value_function
        """
        children = p[1]
        sql = p[1].sql
        p[0] = Node('string_value_function', children, sql)

    def p_character_value_function(self, p):
        """
        ...
        children = p[1]
        sql = p[1].sql
        p[0] = Node('character_value_function', children, sql)

    def p_character_substring_function(self, p):
        """
        character_substring_function : SUBSTRING LPAREN character_value_expression COMMA NUMBER COMMA NUMBER RPAREN
        """
        children = p[3]
        sql = '%s(%s, %s, %s)' % (p[1], p[3].sql, p[5], p[7])
        p[0] = Node('character_substring_function', children, sql)

    def p_fold(self, p):
        """
        fold : UPPER LPAREN character_value_expression RPAREN
             | LOWER LPAREN character_value_expression RPAREN
        """
        children = p[3]
        sql = '%s(%s)' % (p[1], p[3].sql)
        p[0] = Node('fold', children, sql)

    def p_form_of_use_conversion(self, p):
        """
        form_of_use_conversion : CONVERT LPAREN character_value_expression COMMA data_type COMMA NUMBER RPAREN
                               | CONVERT LPAREN character_value_expression COMMA data_type RPAREN
        """
        children = [p[3], p[5]]
        if len(p) == 9:
        ...
        p[0] = Node('form_of_use_conversion', children, sql)

    def p_trim_function(self, p):
        """
        trim_function : LTRIM LPAREN character_value_expression RPAREN
                      | RTRIM LPAREN character_value_expression RPAREN
        """
        children = p[3]
        sql = '%s(%s)' % (p[1], p[3].sql)
        p[0] = Node('trim_function', children, sql)
    ...
    def p_table_expression(self, p):
        ...
        sql_where_clause = ' ' + p[2].sql if p[2].sql else ''
        sql_group_by_clause = ' ' + p[3].sql if p[3].sql else ''
        sql_having_clause = ' ' + p[4].sql if p[4].sql else ''
        sql = '%s%s%s%s' % (p[1].sql, sql_where_clause, sql_group_by_clause, sql_having_clause)
    ...
    def p_null_predicate(self, p):
        ...
        p[0] = Node('null_predicate', children, sql)
    ...
    def p_schema_name(self, p):
        ...
        if len(p) == 2:
            ...
            children = p[3]
            sql = '%s.%s' % (p[1].sql, p[3].sql)
        p[0] = Node('schema_name', children, sql)

    # def p_catalog_name(self, p):
    #     """
    #     catalog_name : identifier
    #     """
    #     sql = p[1].sql
    #     p[0] = Node('catalog_name', None, sql)
    # 
    # def p_unqualified_schema_name(self, p):
    #     """
    #     unqualified_schema_name : identifier
    #     """
    #     sql = p[1].sql
    #     p[0] = Node('unqualified_schema_name', None, sql)
    # 
